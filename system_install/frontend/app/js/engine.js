        ...
        if (err2) {
          ...
          exec("firefox", (err3) => {
            if (err3) {
              ...
              alert(
                "Could not open browser. Make sure a browser is installed.",
              );
            }
          });
        }
...
function open_packup() {
  ...
  exec("packup &", (err) => {
    if (err) {
      ...
      alert(
        "Packup restore functionality is not yet implemented or Packup is not installed.",
      );
    }
  });
}
...
function updateInstallerAltUI() {
  ...
  if (!titleEl || !descEl || !installerCheckbox || !installerCheckbox.checked) {
    ...
    if (descEl) {
      var defDesc =
        descEl.getAttribute("data-default-desc") || descEl.textContent.trim();
  ...
  var defaultTitle =
    titleEl.getAttribute("data-default-title") || titleEl.textContent.trim();
      ...
      diskList.addEventListener("change", function (e) {
        if (
          ...
          if (diskInstallText && selectedDiskName) {
            diskInstallText.textContent =
              'pearOS NiceC0re will be installed on the disk "' +
              selectedDiskName +
              '":';
          }
          updateContinueButtonState();
...
function updateContinueButtonState() {
  ...
  for (var j = 0; j < allButtons.length; j++) {
    if (
      allButtons[j].getAttribute("onclick") &&
      allButtons[j].getAttribute("onclick").includes("select_disk")
    ) {
  ...
  if (
    !hasSelection ||
    (selectedDisk &&
      (selectedDisk.value === "/dev/null" ||
        selectedDisk.getAttribute("data-disk-name") === "NODISK"))
  ) {
...
function select_disk() {
  ...
  checkInternetConnection(function (hasInternet) {
    if (!hasInternet) {
      alert(
        "You need an active internet connection to install pearOS NiceCC0re on your device. Please connect to the internet and try again.",
      );
      return;
    }
...
function select_language() {
  ...
  } else if (strUser == "Español") {
    window.location.href = "lg/es/page_examining.html";
  }
}
